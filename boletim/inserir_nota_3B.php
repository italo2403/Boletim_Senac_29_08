...
<div class="menu-lateral">
    ...
    <div class="container">
    <form action="salvar_notas_3B.php" method="POST">
    <?php
    ...
    $sql_alunos = "SELECT * FROM alunos3B";
    ...
    if ($result_alunos->num_rows > 0) {
        ...
        while ($row_aluno = $result_alunos->fetch_assoc()) {
            ...
            echo '<input type="hidden" name="aluno_id_3b[]" value="' . $row_aluno["aluno_id_3b"] . '">'; // Adiciona campo oculto para o ID do aluno

            // Adicionar campo para selecionar a disciplina
echo '<div class="form-group">';
echo '<label for="disciplina_' . $row_aluno["aluno_id_3b"] . '">Disciplina:</label>';
echo '<select id="disciplina_' . $row_aluno["aluno_id_3b"] . '" name="disciplina[]" required>';
...
echo '</div>';
           ...
           echo '<select id="unidade_' . $row_aluno["aluno_id"] . '" name="unidade[]" >';
           ...
           echo '<div class="form-group">';
           echo '<label for="nota1_' . $row_aluno["aluno_id"] . '">Menção AV1:</label>';
           echo '<input type="text" id="nota1_' . $row_aluno["aluno_id"] . '" name="nota1[]" placeholder="Digite a nota da AV1" >';
           echo '</div>';
           echo '<div class="form-group">';
           echo '<label for="nota2_' . $row_aluno["aluno_id"] . '">Menção AV2:</label>';
           echo '<input type="text" id="nota2_' . $row_aluno["aluno_id"] . '" name="nota2[]" placeholder="Digite a nota da AV2" >';
           echo '</div>';
           ...
           echo '<input type="text" id="conceito_' . $row_aluno["aluno_id"] . '" name="conceito[]" >';
           // echo '<option value="A">D</option>';
           // echo '<option value="ND">ND</option>';
           // echo '</select>';
           echo '<label for="pos_noa_' . $row_aluno["aluno_id"] . '">NOA:</label>';
           echo '<input type="text" id="pos_noa_' . $row_aluno["aluno_id"] . '" name="pos_noa[]" >';
           echo '<label for="mencao_final_anual' . $row_aluno["aluno_id"] . '">Menção Final Anual:</label>';
           echo '<input type="text" id="mencao_final_anual' . $row_aluno["aluno_id"] . '" name="mencao_final_anual[]" >';// mencao_final_anual
           echo '<label for="total_faltas' . $row_aluno["aluno_id"] . '">Total Faltas:</label>';
           echo '<input type="text" id="total_faltas' . $row_aluno["aluno_id"] . '" name="total_faltas[]" >';// total_faltas
           echo '<label for="mencao_final_pos_noa' . $row_aluno["aluno_id"] . '">Menção Final Pos Noa:</label>';
           echo '<input type="text" id="mencao_final_pos_noa' . $row_aluno["aluno_id"] . '" name="mencao_final_pos_noa[]" >';// mencao_final_pos_noa
           echo '<label for="resultado_final_anual' . $row_aluno["aluno_id"] . '">Resultado Final Anual:</label>';
           echo '<input type="text" id="resultado_final_anual' . $row_aluno["aluno_id"] . '" name="resultado_final_anual[]" >';// resultado_final_anual
