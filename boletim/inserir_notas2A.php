<!DOCTYPE html>
    ...
    <div class="menu-lateral">
        ...
        <div class="container">
        <form action="salvar_notas_2A.php" method="POST">
        <?php
        ...
        $sql_alunos = "SELECT * FROM alunos2A";
        ...
        if ($result_alunos->num_rows > 0) {
            ...
            while ($row_aluno = $result_alunos->fetch_assoc()) {
                ...
                echo '<input type="hidden" name="id_aluno[]" value="' . $row_aluno["aluno_id_2a"] . '">'; // Adiciona campo oculto para o ID do aluno

                // Adicionar campo para selecionar a disciplina
    echo '<div class="form-group">';
    echo '<label for="disciplina_' . $row_aluno["aluno_id_2a"] . '">Disciplina:</label>';
    echo '<select id="disciplina_' . $row_aluno["aluno_id_2a"] . '" name="disciplina[]" required>';
    ...
    echo '</div>';
                ...
                echo '<div class="form-group">';
                echo '<label for="unidade_' . $row_aluno["aluno_id_2a"] . '">Unidade:</label>';
                echo '<select id="unidade_' . $row_aluno["aluno_id_2a"] . '" name="unidade[]" >';
                ...
                echo '<div class="form-group">';
                echo '<label for="nota1_' . $row_aluno["aluno_id_2a"] . '">Menção AV1:</label>';
                echo '<input type="text" id="nota1_' . $row_aluno["aluno_id_2a"] . '" name="nota1[]" placeholder="Digite a nota da AV1" >';
                echo '</div>';
                echo '<div class="form-group">';
                echo '<label for="nota2_' . $row_aluno["aluno_id_2a"] . '">Menção AV2:</label>';
                echo '<input type="text" id="nota2_' . $row_aluno["aluno_id_2a"] . '" name="nota2[]" placeholder="Digite a nota da AV2" >';
                echo '</div>';

                // TEM QUE TROCAR PARA NÃO SER O SELECT, MAS SÓ CAMPO DE TEXT EM TODAS AS INSERÇÕES DE VALORES
                // EM TODAS AS TURMAS. FAZER COM URGÊNCIA DE DEPOIS EXECUTAR OS TESTES

                // Adicionar input de conceito
                echo '<div class="form-group">';
                echo '<label for="conceito_' . $row_aluno["aluno_id_2a"] . '">Conceito:</label>';
                echo '<input type="text" id="conceito_' . $row_aluno["aluno_id_2a"] . '" name="conceito[]" >';
                // echo '<option value="A">D</option>';
                // echo '<option value="ND">ND</option>';
                // echo '</select>';
                echo '<label for="pos_noa_' . $row_aluno["aluno_id_2a"] . '">NOA:</label>';
                echo '<input type="text" id="pos_noa_' . $row_aluno["aluno_id_2a"] . '" name="pos_noa[]" >';
                echo '<label for="mencao_final_anual' . $row_aluno["aluno_id_2a"] . '">Menção Final Anual:</label>';
                echo '<input type="text" id="mencao_final_anual' . $row_aluno["aluno_id_2a"] . '" name="mencao_final_anual[]" >';// mencao_final_anual
                echo '<label for="total_faltas' . $row_aluno["aluno_id_2a"] . '">Total Faltas:</label>';
                echo '<input type="text" id="total_faltas' . $row_aluno["aluno_id_2a"] . '" name="total_faltas[]" >';// total_faltas
                echo '<label for="mencao_final_pos_noa' . $row_aluno["aluno_id_2a"] . '">Menção Final Pos Noa:</label>';
                echo '<input type="text" id="mencao_final_pos_noa' . $row_aluno["aluno_id_2a"] . '" name="mencao_final_pos_noa[]" >';// mencao_final_pos_noa
                echo '<label for="resultado_final_anual' . $row_aluno["aluno_id_2a"] . '">Resultado Final Anual:</label>';
                echo '<input type="text" id="resultado_final_anual' . $row_aluno["aluno_id_2a"] . '" name="resultado_final_anual[]" >';// resultado_final_anual
