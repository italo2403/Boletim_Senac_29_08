...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["id_aluno"]) && isset($_POST["unidade"]) && isset($_POST["nota1"]) && isset($_POST["nota2"]) && isset($_POST["conceito"]) && isset($_POST["pos_noa"]) && isset($_POST["disciplina"])) {
        ...
        $sql_insert_notas = "INSERT INTO notas_2A (aluno_id_2a, id_disciplina_2A, unidade, av1, av2, conceito, pos_noa) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        if ($stmt) {
            ...
            $stmt->bind_param("iiissss", $id_aluno, $id_disciplina_2A, $unidade, $nota1, $nota2, $conceito, $pos_noa);

            // Itera sobre os dados recebidos do formulário
            for ($i = 0; $i < count($_POST["id_aluno"]); $i++) {
                $id_aluno = $_POST["id_aluno"][$i];
                $id_disciplina_2A = $_POST["disciplina"][$i];
...
$sql_select_nome = "SELECT nome_aluno FROM alunos2A WHERE aluno_id_2a = ?";
